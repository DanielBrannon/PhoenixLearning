...
function Study() {
  ...
  const fetchInitialQuestions = async () => {
    ...
    try {
      ...
      console.log(
        'Fetched questions with image_urls:',
        questions.map((q) => ({ id: q.id, image_url: q.image_url }))
      );
  ...
  const handleBatchSubmit = async (e) => {
    ...
    const progressPromises = questionsQueue.map(async (question) => {
      ...
      batchResults[question.id] = isCorrect
        ? 'Correct!'
    ...
    return (
      <div className="study-container">
        ...
        <div className="study-card">
          ...
          <p>
            You got {reviewResults.filter((r) => r.correct).length} out of {reviewResults.length}{' '}
            correct!
          </p>
        ...
        <div className="study-card">
          ...
          {(currentQuestion.choice1 ||
            ...
            <div>
              ...
              {choiceFeedback && (
                <p style={{ color: choiceFeedback.includes('Correct') ? 'green' : 'red' }}>
                  {choiceFeedback}
                </p>
              )}
            </div>
